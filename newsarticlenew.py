...
# Print sentiment scores for each element in the Series
for i, sentiment_scores in enumerate(sentiments):
    print(f"Element {i+1} Sentiment Scores: {sentiment_scores}")
...
# Count variables for each sentiment
positive_count = 0
neutral_count = 0
negative_count = 0
# Lists to store data for each sentiment
all_data = []
threshold = 0.05
# Iterate through sentiment scores and categorize sentiments
for i, sentiment_scores in enumerate(sentiments):
    ...
    if compound_score >= threshold:
        sentiment = 'Positive'
        positive_count += 1
    elif compound_score > -threshold and compound_score < threshold:
        sentiment = 'Neutral'
        neutral_count += 1
    else:
        sentiment = 'Negative'
    ...
    all_data.append((i + 1, merged_df['Summarize'].iloc[i], sentiment, sentiment_scores))

# Convert list to DataFrame
all_df = pd.DataFrame(all_data, columns=['Element', 'Summarize', 'Sentiment', 'Sentiment Scores'])

# Save DataFrame to a CSV file
all_df.to_csv('all_sentiments1.csv', index=False)
